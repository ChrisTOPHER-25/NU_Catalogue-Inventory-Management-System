...
class SalesController extends CI_Controller {
	...
	public function reports() {
		...
		if ($from && $to) {
			$sales = $this->db->where('DATE_FORMAT(date_time, "%Y-%m-%d") >=', $from)
						...
						->like('customer_id', $customer_id, 'both')
						->order_by('id', 'DESC')
		...
		foreach ($sales as $sale) {
			...
			$customer = $this->db->where('id', $sale->customer_id)->get('customers')->row();
			$sub_total = 0;
	...
	public function togglePaymentStatus() {
		...
		$this->db->where('id', $id)->update('sales', ['payment_status' => $status]);
		return redirect(base_url('/sales'));
	}
